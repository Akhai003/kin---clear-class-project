...
const Story = ({ article, compact=false }: {article:any, compact?:boolean}) => (
  <Link to={`/article/${article.slug}`} className={`group ${compact ? 'grid grid-cols-[120px_1fr] gap-5 items-center' : 'block'}`}>
  ...
  return (
    <div className="bg-ivory min-h-screen pt-32 pb-24">
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12">
        ...
        {featured && <motion.section initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} className="mb-20 lg:mb-24"><Link to={`/article/${featured.slug}`} className="group grid lg:grid-cols-12 bg-white rounded-[26px] overflow-hidden border border-sage-light"><div className="lg:col-span-7 aspect-[16/10] lg:aspect-auto min-h-[420px] overflow-hidden"><img src={featured.image} alt={featured.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-[1.02]" /></div><div className="lg:col-span-5 p-8 lg:p-12 flex flex-col justify-center"><span className="text-sage text-xs font-semibold tracking-widest uppercase mb-4">Featured / {featured.category}</span><h2 className="text-fluid-h2 mb-5 group-hover:text-sage transition-colors">{featured.title}</h2><p className="text-fluid-body text-slate mb-8">{featured.excerpt}</p><div className="flex items-center gap-5 text-sm"><span className="font-medium inline-flex items-center gap-1 link-underline pb-1">Read Story <ArrowRight className="w-4 h-4" /></span><span className="text-slate">{featured.readTime}</span></div></div></Link></motion.section>}
